import { PrismaClient, Role, SupplierStatus, ProductStatus } from '@prisma/client';
...
async function main() {
  ...
  const suppliersData = [
    { email: 'supplier1@example.com', name: 'Textile Hub', status: SupplierStatus.VERIFIED },
    { email: 'supplier2@example.com', name: 'Chemical Corp', status: SupplierStatus.UNDER_REVIEW },
    { email: 'supplier3@example.com', name: 'Raw Materials Ltd', status: SupplierStatus.SUBMITTED },
  ];
  ...
  if (supplier1) {
    // Check if products already exist to avoid duplicates if re-running
