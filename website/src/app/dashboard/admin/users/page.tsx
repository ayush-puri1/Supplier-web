...
export default function UserManagementPage() {
  ...
  const roleBadge = (role: string) => {
    const styles: Record<string, string> = { SUPER_ADMIN: 'bg-purple-50 text-purple-700 border-purple-200', ADMIN: 'bg-blue-50 text-blue-600 border-blue-200', SUPPLIER: 'bg-blue-50 text-emerald-700 border-emerald-200' };
    ...
    <DashboardLayout title="User Management">
      <div className="space-y-8 animate-fade-in-up">
        ...
        <div className="flex items-center justify-between">
          ...
          <button onClick={() => setShowCreate(!showCreate)} className={`px-5 py-2.5 rounded-full text-sm font-semibold transition-all ${showCreate ? 'bg-zinc-100 text-zinc-600 border border-zinc-200' : 'bg-[#2563EB] text-white hover:bg-[#1D4ED8] shadow-lg shadow-[#2563EB]/20'}`}>
        ...
        {showCreate && (
          <div className="animate-fade-in-down">
            <form onSubmit={handleCreateAdmin} className="bg-gradient-to-r from-[#ECFDF5] to-white rounded-2xl border border-[#2563EB]/30 p-6">
              <h3 className="font-semibold text-[#0F1117] mb-4">Create New Admin Account</h3>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 items-end">
                <div><label className="block text-xs font-semibold text-[#374151] mb-1.5">Email</label><input type="email" value={newEmail} onChange={(e) => setNewEmail(e.target.value)} placeholder="admin@delraw.com" required className="w-full px-4 py-3 rounded-xl border border-[#E5E7EB] text-sm focus:border-[#2563EB] focus:ring-2 focus:ring-[#2563EB]/20 outline-none" /></div>
                <div><label className="block text-xs font-semibold text-[#374151] mb-1.5">Password</label><input type="password" value={newPassword} onChange={(e) => setNewPassword(e.target.value)} placeholder="Min 6 characters" required minLength={6} className="w-full px-4 py-3 rounded-xl border border-[#E5E7EB] text-sm focus:border-[#2563EB] focus:ring-2 focus:ring-[#2563EB]/20 outline-none" /></div>
                <button type="submit" disabled={creating} className="py-3 rounded-xl bg-[#2563EB] text-white font-semibold text-sm hover:bg-[#1D4ED8] transition-all disabled:opacity-50">{creating ? 'Creating...' : 'Create Admin'}</button>
              </div>
            </form>
          </div>
        )}
                  ...
                  <tbody className="divide-y divide-[#E5E7EB]/50">
                    {admins.map((u) => (
                      <tr key={u.id} onClick={() => { setSelected(u); setResetPassword(''); }} className={`hover:bg-[#F9FAFB] cursor-pointer transition-colors ${selected?.id === u.id ? 'bg-[#ECFDF5]/50' : ''}`}>
                        ...
                        <td className="px-6 py-4"><span className={`inline-flex items-center gap-1 text-[10px] font-bold uppercase ${u.isActive !== false ? 'text-blue-600' : 'text-red-600'}`}><span className={`w-1.5 h-1.5 rounded-full ${u.isActive !== false ? 'bg-blue-500' : 'bg-red-500'}`} />{u.isActive !== false ? 'Active' : 'Suspended'}</span></td>
                        <td className="px-6 py-4 text-right"><button disabled={actionLoading || u.id === currentUser?.id} onClick={(e) => { e.stopPropagation(); handleToggleActive(u.id, u.isActive === false); }} className={`px-3 py-1.5 rounded-lg text-[10px] font-bold transition-colors disabled:opacity-30 ${u.isActive !== false ? 'bg-red-50 text-red-600 hover:bg-red-100' : 'bg-blue-50 text-blue-600 hover:bg-emerald-100'}`}>{u.isActive !== false ? 'Suspend' : 'Activate'}</button></td>
                      </tr>
                    ))}
                  </tbody>
                </table>
              ...
              <tbody className="divide-y divide-[#E5E7EB]/50">
                {suppliers.map((u) => (
                  <tr key={u.id} className="hover:bg-[#F9FAFB] transition-colors">
                    ...
                    <td className="px-6 py-4"><span className={`text-[10px] font-bold ${u.isActive !== false ? 'text-blue-500' : 'text-red-500'}`}>{u.isActive !== false ? '● Active' : '● Suspended'}</span></td>
