    ...
    <DashboardLayout title="Admin">
      <div className="space-y-8 animate-fade-in-up">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          ...
          <StatCard label="Live Products" value={stats?.products?.live || 0} subtitle="Active inventory" valueColor="text-blue-600" icon={<Package className="w-5 h-5" />} />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div className="bg-white rounded-2xl border border-[#E5E7EB] overflow-hidden">
            <div className="flex items-center justify-between px-6 py-4 border-b border-[#E5E7EB]">
              ...
              <Link href="/dashboard/admin/suppliers" className="text-xs font-semibold text-[#2563EB] hover:underline flex items-center gap-1">View All <ArrowRight className="w-3 h-3" /></Link>
            </div>
                  ...
                  {pendingSuppliers.map((s) => (
                    <tr key={s.id} className="hover:bg-[#F9FAFB] transition-colors">
                      ...
                      <td className="px-6 py-3 text-right space-x-2">
                        ...
                        <button onClick={() => handleSupplierAction(s.id, 'VERIFIED')} className="px-3 py-1.5 rounded-lg text-[10px] font-bold bg-blue-50 text-emerald-700 hover:bg-emerald-100 transition-colors">Approve</button>
          ...
          <div className="bg-white rounded-2xl border border-[#E5E7EB] overflow-hidden">
            <div className="flex items-center justify-between px-6 py-4 border-b border-[#E5E7EB]">
              ...
              <Link href="/dashboard/admin/products" className="text-xs font-semibold text-[#2563EB] hover:underline flex items-center gap-1">View All <ArrowRight className="w-3 h-3" /></Link>
            </div>
                ...
                <tbody className="divide-y divide-[#E5E7EB]/50">
                  {pendingProducts.map((p) => (
                    <tr key={p.id} className="hover:bg-[#F9FAFB] transition-colors">
                      ...
                      <td className="px-6 py-3 text-right space-x-2">
                        <button onClick={() => handleProductAction(p.id, 'LIVE')} className="px-3 py-1.5 rounded-lg text-[10px] font-bold bg-blue-50 text-emerald-700 hover:bg-emerald-100 transition-colors">Approve</button>
